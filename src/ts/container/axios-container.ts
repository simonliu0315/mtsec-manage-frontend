...
export function createNetworkAxios(axiosConfig: AxiosConfig): AxiosInstance {
  ...
  axios.interceptors.request.use(
    function (config) {
      console.log('loadingMasker show', config)
      loadingMasker.show();
      if (config.headers) {
        const authStore =  useAuthStore();      
        if (authStore.isAuthorized) {
          if (authStore.authorization) {
            config.headers["Authorization"] = authStore.authorization;
            console.log("set header Authorization:", authStore.authorization);
          }
        }
      }
  ...
  axios.interceptors.response.use(
    ...
    (error) => {
      ...
      if (!error.response) {
        ...
        alerter.alert("NET-HTTP-0403-E", [], dismissIn, traceId);
      } else {
...
function escapeHtml(message: string): string {
  const s = (message != undefined ? message.split("//") : "https://www.cht.com.tw".split("//"));
  let url120 = "";
